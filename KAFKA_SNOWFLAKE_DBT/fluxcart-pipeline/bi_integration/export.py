...
def upload_to_snowflake(file_path, table_name):
    ...
    try:
        conn = snowflake.connector.connect(
            ...
            account='PWJKGQP-AX00621',   # example: ab12345.eu-central-1
            warehouse='COMPUTE_WH_FLUX',
            database='FLUXCART',
            schema='RAW',
            role='TRANSFORM'
        )
        ...
        cs.execute(f"PUT 'file:///{safe_path}' @fluxcart_stage AUTO_COMPRESS=TRUE OVERWRITE=TRUE")

        # Load data into table
        cs.execute(f"""
        COPY INTO {table_name}
        FROM @fluxcart_stage/{os.path.basename(file_path)}
        FILE_FORMAT=(TYPE=CSV SKIP_HEADER=1 FIELD_OPTIONALLY_ENCLOSED_BY='"')
        ON_ERROR='CONTINUE'
        """)

        print(f"[snowflake] loaded {table_name}")
...
upload_to_snowflake(ANALYTICS_CSV, "RAW_ANALYTICS")
upload_to_snowflake(FRAUD_CSV, "RAW_FRAUD")
upload_to_snowflake(INVENTORY_CSV, "RAW_INVENTORY")
